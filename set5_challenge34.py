...
class A():
    ...
    def send_params(self):
        return p, g, self.DH.get_pubkey()

    def recv_params(self, Bval):
...
class M():
    ...
    def recv_params_B(self, Bval):
        self.Bval = Bval

    def send_params_A(self):
        return p
...
if __name__ == '__main__':
    ...
    # Bs = B.keygen(p)
    # As = A.keygen(p)
    # Ab = As.to_bytes(16, byteorder='big')
    # Bb = Bs.to_bytes(16, byteorder='big')
    # print(f'Ab={Ab}')
    # print(f'Bb={Bb}')
    # Akey = sha1(Ab).to_bytes(20, byteorder='big')[:16]
    # Bkey = sha1(Bb).to_bytes(20, byteorder='big')[:16]
    # print(f'Akey={Akey}')
    # print(f'Bkey={Bkey}')
    # Aiv = os.urandom(16)
    # Biv = os.urandom(16)
    # msg = b'Test Message, '*11
    # Amsg = encrypt_aes_cbc(msg, Akey, Aiv) + Aiv
    # Bmsg = encrypt_aes_cbc(msg, Bkey, Biv) + Biv

    # Correctly functioning protocol with no MITM
    Abot = A(p, g)
    p, g, Aval = Abot.send_params()
    Bbot = B(p, g, Aval)
    Abot.recv_params(Bbot.send_params())
    Actext = Abot.send_msg()
    Bbot.recv_msg(Actext)
    Bctext = Bbot.send_msg()
    print(f'Actext={Actext}')
    print(f'Bctext={Bctext}')

    Abot = A(p, g)
    p, g, Aval = Abot.send_params()
    Mbot = M(p, g, Aval)
    Bbot = B(*Mbot.send_params_B())
    Mbot.recv_params_B(Bbot.send_params())
    Abot.recv_params(Mbot.send_params_A())
    Actext = Abot.send_msg()
    Mbot.recv_msg_A(Actext)
    MActext = Mbot.send_msg_B()
    Bbot.recv_msg(MActext)
    Bctext = Bbot.send_msg()
    Mbot.recv_msg_B(Bctext)
    MBctext = Mbot.send_msg_A()
    # Abot.recv_msg(MBctext)
